...
MACHINE=builderbot
IMAGE_FILE_NAME=console-image-builderbot-${MACHINE}.tar.xz
...
if [ -z "$OETMP" ]; then
	echo -e "\nWorking from local directory, or export OETMP=<your-poky-build-dir>, example: export OETMP=~/poky-sumo/build/tmp"
	SRCDIR=.
else
	echo -e "\nOETMP: $OETMP"
...
if [ ! -f "${SRCDIR}/${IMAGE_FILE_NAME}" ]; then
        echo "File not found: ${SRCDIR}/${IMAGE_FILE_NAME}"
        exit 1
fi
...
echo "Extracting ${IMAGE_FILE_NAME} to /media/card"
sudo tar -C /media/card -xJf ${SRCDIR}/${IMAGE_FILE_NAME}
